...
export const Image = ({img, deleteImageCallback}: ImagePropsType) => {
    ...
    return (
        <div className={styles.Image}>
            <button onClick={deleteOnClickHandler} className={styles.button}>X</button>
            <img src={img.thumbnailUrl} alt='' onClick={setShowOnClickHandler}/>
            <div><b>albumId: </b>{img.albumId}</div>
            <div><b>Id: </b>{img.id}</div>
            <div className={styles.Title}><b>Title: </b>{img.title}</div>
            <Modal enableBackground={true}
